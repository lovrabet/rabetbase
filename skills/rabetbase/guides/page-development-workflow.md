...
本工作流只适用于 **DOV2 标准页面** 路径：

- 基于数据集首次生成标准页
- 数据集字段变更后同步已有标准页
- 将标准页 schema 拉到本地进行 IDE 修改，再推回平台
...
评论、附件、历史记录等子资源，默认不优先作为独立主页面；若业务目标不是标准 CRUD 后台，先停下来确认是否还应继续走 `page` 工作流。

## 核心原则

1. 标准页面是“标准后台”方案，不是所有数据集的默认答案。
