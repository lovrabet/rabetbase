...
export default async function sendOrderNotification(params, context) {
  ...
  return await context.client.extension.execute("notification", "send", {
    configCode: CONFIG_CODE,
    audiences: ALLOWED_AUDIENCES,
    message: {
      title: "订单状态通知",
      summary: "订单 " + orderNo + " 已进入待处理状态",
      theme: "blue",
    },
  });
}
...
从 `data.configs[]` 中根据 `configName` 和 `description` 选择配置，把该项 `configCode` 固定到受控业务脚本中。查询命令只返回安全身份与描述字段，不返回 `channelConfig`、`endpointUrl` 或凭据。
...
扩展调用对象只允许以下字段，多余字段会直接失败：

| 字段 | 必填 | 约束 |
|------|------|------|
| `configCode` | 是 | 当前应用中已存在且未删除的应用级通知渠道配置编码；先用 `rabetbase notification config-list` 查询，不要猜测 |
| `audiences` | 按渠道 | 对象数组；EMAIL 和飞书应用机器人必须至少解析出一个接收人，Webhook 机器人可以不传或传空数组 |
| `message` | 是 | 消息对象，字段严格按下表传递 |

`appCode` 和当前用户由可信脚本上下文注入。调用对象中禁止传 `appCode`、`datasetCode`、`sceneCode`、`endpointUrl`、`channelConfig`、token、secret 或其他旧通知字段。`configCode` 只能解析当前应用的配置，不能借此跨应用发送。
...
```javascript
[
  { type: "USER", ids: ["user@example.com"] },
  { type: "ROLE", codes: ["ADMIN"] },
]
```

| `type` | 值字段 | 说明 |
|--------|--------|------|
| `USER` | `ids` | 非空字符串数组；具体标识必须与目标渠道的接收人类型一致，EMAIL 可使用邮箱或可解析的应用用户标识 |
| `ROLE` | `codes` | 非空字符串数组；使用当前应用已确认的角色编码或角色标识，不要猜测 |

`type` 只支持 `USER` / `ROLE`，且对象不能混入其他字段。EMAIL 和飞书应用机器人若最终没有解析出有效接收人，会在发送前失败；不要把“传了 audiences”当作“接收人已解析”。
...
| `title` | 是 | 非空字符串 |
| `summary` | 是 | 非空字符串 |
| `theme` | 否 | `blue` / `green` / `orange` / `red` / `grey` |
| `detailMarkdown` | 否 | 字符串 |
...
{
  "sent": true,
  "configCode": "<confirmed-config-code>",
  "channelType": "EMAIL",
  "message": "通知发送成功"
}
```
...
* [ ] 若脚本涉及数据集，已执行 `rabetbase dataset detail` 获取数据集信息
...
* [ ] 若脚本发送通知，只使用 `configCode` / `audiences` / `message`，且已确认接收对象和外部副作用
