...
### 动态值生成 JavaScript / TypeScript 标识符

- 外部动态值不得未经处理直接用于 JavaScript / TypeScript 标识符。
- 生成名称必须合法、确定且不存在歧义；无法安全生成时应明确失败。
- 已发布的公开名称应保持兼容，变更时按破坏性变更处理。
- 生成文件不应手工修补，应修正规则或输入后重新生成。
