...
- `relation-create`：确认关系已存在，且 `to-label-column` 与请求一致；如果 readback 暴露 `cardinality`，也会一并校验。
- `relation-update`：确认关系仍存在，且 `to-label-column` 与请求一致；如果 readback 暴露支持的 `cardinality`，也会一并校验。
...
`before` / `after` / `verification.afterReadback` 表示写入前后可稳定确认的关系事实。定位关系时使用 `from.table`、`from.column`、`to.table`、`to.column`。
